...
export function FeaturesSection() {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen w-full py-20 bg-upslp-600 overflow-hidden">
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" style={{ y: cardsY }}>
          {features.map((feature, index) => (
            <motion.div
              ...
              className="bg-upslp-700/50 backdrop-blur-sm p-6 rounded-xl border border-upslp-500/50 hover:border-orange-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-upslp-900/20"
            >
              <div className="mb-4">{feature.icon}</div>
              <h3 className="text-xl font-semibold text-white mb-2">{feature.title}</h3>
              <p className="text-upslp-100">{feature.description}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
