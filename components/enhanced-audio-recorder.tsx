...
export function EnhancedAudioRecorder({ onRecordingComplete, disabled = false }: EnhancedAudioRecorderProps) {
  ...
  const visualizeAudio = () => {
    ...
    const draw = () => {
      ...
      canvasCtx.fillStyle = "rgb(26, 43, 95)" // Color de fondo UPSLP
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        const hue = 32 - (dataArray[i] / 255) * 10 // Naranja a amarillo
  ...
  return (
    <Card className="w-full bg-upslp-800/30 border border-upslp-700/30">
      <CardHeader>
        <CardTitle className="flex items-center justify-between text-white">
          ...
          {isRecording && (
            <div className="flex items-center space-x-2">
              <span
                className={`h-2 w-2 rounded-full ${isPaused ? "bg-yellow-500" : "bg-orange-500 animate-pulse"}`}
              ></span>
      ...
      <CardContent className="space-y-4">
        ...
        {showSegmentInfo && segments.length > 0 && (
          <Alert className="bg-upslp-700/50 border-upslp-600 text-white">
            <AlertCircle className="h-4 w-4" />
        ...
        <div className="flex flex-col items-center">
          <div
            className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full flex items-center justify-center ${
              isRecording
                ? isPaused
                  ? "bg-yellow-600/30"
                  : `bg-orange-500/30 ${audioLevel > 0.05 ? "animate-pulse" : ""}`
                : disabled
                  ? "bg-gray-600/30"
                  : "bg-upslp-600/30"
            }`}
            style={{
              boxShadow:
                isRecording && !isPaused && audioLevel > 0.05
                  ? `0 0 ${20 + audioLevel * 30}px ${audioLevel * 15}px rgba(247, 148, 29, ${audioLevel * 0.5})`
                  : "none",
            }}
          >
            <Mic
          ...
          <div className="mt-4 text-center">
            ...
            <div className="text-upslp-300/70 text-xs sm:text-sm">
              {isRecording
                ? isPaused
                  ? "Grabación pausada"
                  : "Grabando..."
            ...
            {segments.length > 0 && (
              <div className="text-upslp-300/70 text-xs mt-1">
        ...
        <div className="w-full">
          <canvas ref={canvasRef} width="600" height="100" className="w-full h-16 sm:h-24 rounded-lg bg-upslp-700/50" />
        </div>

        {isRecording && (
          <div className="space-y-1">
            <div className="flex justify-between text-xs text-upslp-300/70">
              <span>Progreso del segmento actual</span>
              <span>{Math.round(segmentProgress)}%</span>
            </div>
            <Progress value={segmentProgress} className="h-1 bg-upslp-700/50" indicatorClassName="bg-orange-500" />
          </div>
        )}
      </CardContent>
      ...
      <CardFooter className="flex justify-center gap-3 flex-wrap">
        {!isRecording ? (
          <Button
            className={`${
              disabled ? "bg-gray-600 hover:bg-gray-700 cursor-not-allowed" : "bg-upslp-600 hover:bg-upslp-700"
            } text-white`}
          ...
          <>
            {!isPaused ? (
              ...
              <Button
                variant="outline"
                className="border-upslp-600/50 text-upslp-300 hover:bg-upslp-800/30"
            ...
            <Button
              variant="outline"
              className="border-orange-600/50 text-orange-400 hover:bg-orange-800/30"
