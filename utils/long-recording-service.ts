export class LongRecordingService {
  private mediaRecorder: MediaRecorder | null = null
  private stream: MediaStream | null = null
  private segments: Blob[] = []
  private currentSegment: Blob[] = []
  private segmentDuration = 10 * 60 * 1000 // 10 minutos por segmento (ajustable)
  ...
  async startRecording(audioConstraints: MediaTrackConstraints = {}) {
    try {
      ...
      // Notificar cambio de estado
      this.notifyStatusChange("configuring")

      // Configurar el MediaRecorder con la mejor calidad disponible
  ...
  private startSegmentTimer() {
    ...
    this.segmentTimer = setTimeout(() => {
      if (this.isRecording && this.mediaRecorder && this.mediaRecorder.state === "recording") {
        // Notificar cambio de estado
        this.notifyStatusChange("switching_segment")
  ...
  stopRecording(): Promise<Blob> {
    return new Promise((resolve) => {
      this.notifyStatusChange("finalizing")
  ...
  // Obtener el tamaño total estimado en MB
  getTotalSize(): number {
    let totalSize = 0
